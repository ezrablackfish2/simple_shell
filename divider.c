...
char **toke(char *inputString, char *delimiters)
{
    ...
    for (index1 = 0; inputString[index1] != '\0'; index1++)
        if (!is_delim(inputString[index1], delimiters) && (is_delim(inputString[index1 + 1], delimiters) || !inputString[index1 + 1]))
            numWords++;
    ...
    {
        while (is_delim(inputString[index2], delimiters))
            index2++;
        index4 = 0;
        while (!is_delim(inputString[index2 + index4], delimiters) && inputString[index2 + index4])
            index4++;
